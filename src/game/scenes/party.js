...
class PartyScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.images = {
      ...
      banner: { level: 2, obj: this.add.image(centerX, centerY, 'banner') },
      nina: { level: 2, obj: this.add.image(centerX, centerY, 'nina') },
      tv: { level: 1, obj: this.add.image(centerX, centerY, 'tv') },
      ...
      crowd1: { level: -2, obj: this.add.image(centerX, centerY, 'crowd1') },
      crowd2: { level: -2, obj: this.add.image(centerX, centerY, 'crowd2') },
    };
    ...
    this.input.on('pointermove', (pointer) => {
      Object.values(this.images).forEach((image) => {
        const newX = centerX + ((pointer.x - centerX) * (0.03 * image.level));
        const newY = centerY + ((pointer.y - centerY) * (0.015 * image.level)) - 20;
